#!/usr/bin/env python

# Edited by amiro and eterzic 21.02.2021
from __future__ import print_function, division

import numpy as np
import matplotlib.pyplot as plt

import shapefile

FILENAME = '/home/arnaumiro/projects/DATA/ShapeFiles/WorldCountries/World_Countries'

sf  = shapefile.Reader(FILENAME)

# Loop every country
for idx in range(len(sf)):
	rec = sf.record(idx)
	if 'Montenegro' in rec[0]: print(idx,rec[0])

# Extract Montenegro
print(sf.record(153))
shp = sf.shape(153)
xy  = np.array(shp.points)
plt.plot(xy[:,0],xy[:,1],'.k')

# Detect jump
d    = np.diff(xy,axis=0)
norm = np.sqrt(np.sum(d*d,axis=1))
ids  = np.where(norm > 0.03*np.max(norm))[0]
st   = xy.shape[0]#ids[0]+1
print(ids,st)
plt.plot(xy[:st,0],xy[:st,1],'k')

# Save data as xyz points
xyz = np.zeros((st,3),dtype=np.double)
xyz[:,0] = xy[:st,0]
xyz[:,1] = xy[:st,1]

np.save('../Basins/shapes/Montenegro_WorldCountries.npy',xyz)

plt.show()